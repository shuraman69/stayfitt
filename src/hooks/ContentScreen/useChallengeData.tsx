...
import { Lightning } from '../../screens/ContentScreen/components/svg/Lightning'
...
export const useChallengeData = (item: ChallengeNodeI) => {
  ...
  const progressText = isDone ? `Получено ${node.bonusPoints}` : `Еще ${getDayDiff(node.endDate)} ${plural(getDayDiff(node.endDate), 'день', 'дня', 'дней')}`
